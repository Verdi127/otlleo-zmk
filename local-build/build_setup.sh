...
setup_sandbox() {
  ...
  # Copy otello/ conf files to top-level config location so ZMK finds them.
  for f in "$SANDBOX_ROOT/$CONFIG_PATH/otello/"*.conf; do
...
build_firmware() {
  ...
  local extra_snippet=""
  if [[ "$ENABLE_USB_LOGGING" != "true" ]] && [[ "$target" == "otello_right_bt" || "$target" == "otello_dongle" ]]; then
  ...
  # Pass the boards/ module so ZMK can discover custom shields.
  ...
  # For stacked shields (space-separated), pass the full string to -DSHIELD.
  # Substitute the discovered target overlay name in place of the primary shield token.
  local dshield
  if [[ "$shield" == *" "* ]]; then
  ...
  # "bt"                   - use otello_bt
  # "dongle_standard_nano" - use otello_dongle
  case "$entry_format" in
    bt)                    format_dir="otello_bt" ;;
    dongle_standard_nano)  format_dir="otello_dongle" ;;
  ...
  for build_json in "${build_items[@]}"; do
    ...
    for entry_board in "${build_boards[@]}"; do
      ...
      if [[ "$shield" == "settings_reset" ]]; then
        ...
        # For stacked shields (space-separated),
